---
name: 期刊摘要與PDF工作流
description: Process yearly journal-article Excel trackers for Chinese-language research workflows. Use when Codex needs to fill missing abstracts, supplement official PDF/source URLs, download public PDFs, generate per-year manifests/reports, package files for manual review, or perform final cleanup across yearly batches without overwriting the original Excel files.
---

# Journal Abstract PDF Workflow
...
Treat Yuandan / 月旦 as auxiliary only. If it looks like 正文首段 instead of a formal abstract, do not adopt it as the abstract.
...
6. Generate updated Excel copy, yearly manifest, and yearly short report.
...
Keep the end-of-run report short and operational. Include year handled, total rows, downloaded PDFs, no public PDF, login/paywalled items, ambiguous items, failed downloads, output Excel path, manifest path, report path, and handoff PDF folder path.

## Guardrails For Long Runs

If the task spans many years or many rows: run year by year, prefer a fresh window for the next year when context starts to bloat, preserve intermediate files after each year, and do not silently change schemas halfway through the series.
